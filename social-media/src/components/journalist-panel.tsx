            ...
            {[...activeVariant.layers]
              ...
              .map((layer) => {
                    ...
                    >
                      {source ? (
                        slot?.kind === "video" || layer.kind === "video" ? (
                          <video src={source} muted playsInline autoPlay loop />
                        ) : (
                          <img src={source} alt="" />
                        )
                      ) : (
                ...
                const alignmentClass =
                  style.placement.includes("right") ? "right" : "left";
                const verticalClass = style.placement.includes("bottom") ? "bottom" : "top";

                return (
                  <div
                    key={layer.id}
                    className={`journalist-preview-layer journalist-preview-text ${alignmentClass} ${verticalClass}`}
                    style={{
                      left: `${layer.x}%`,
                      top: `${layer.y}%`,
                      width: `${layer.width}%`,
                      height: `${layer.height}%`,
                      zIndex: layer.zIndex + 1,
                      justifyItems: alignmentClass === "right" ? "end" : "start",
                    }}
                  >
